...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.save_every = 25 #CERV: I CHANGED PERIODIC CHECKPOINT SAVING
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        if dim == 2:
            ...
            if max(patch_size) / min(patch_size) > 1.5: #  meaning a rectangular patch size
                rotation_for_DA = (-15. / 360 * 2. * np.pi, 15. / 360 * 2. * np.pi)
            else: #square patch size
    ...
    def get_training_transforms(
        ...
        transforms.append(
            SpatialTransform(
                patch_size_spatial, patch_center_dist_from_border=0, random_crop=False, p_elastic_deform=0.15, #CERV: I ENABLED ELASTIC DEFORM
                p_rotation=0.2,
 ...
 # #region ended
  ...
  # region added
    def save_augmented_samples(self, num_samples: int = 5):
        ...
        save each image/label pair as a .npz file, and (if SimpleITK is
        available) also as .nii.gz so you can open them in a viewer.
        Output lands in  <output_folder>/augmented_samples/
        """
        ...
        save_dir = join(self.output_folder, 'augmented_samples')
        maybe_mkdir_p(save_dir)
        self.print_to_log_file(f"Saving {num_samples} augmented samples to {save_dir}")

        for i in range(num_samples):
            batch = next(self.dataloader_train)
            data  = batch['data']           # (B, C, [D,] H, W)  – torch or numpy
            target = batch['target']

            # target can be a list (deep supervision) or a single tensor
            if isinstance(target, list):
                seg = target[0]             # highest-resolution output
            else:
                seg = target
            ...
            # save the whole batch as .npz so nothing is lost
            npz_path = join(save_dir, f'aug_batch_{i:03d}.npz')
            np.savez_compressed(npz_path, data=data_np, seg=seg_np)
            self.print_to_log_file(f"  saved {npz_path}  data{data_np.shape}  seg{seg_np.shape}")

            # optionally save per-sample NIfTI files (first sample in each batch)
            if have_sitk:
                d0  = data_np[0]           # (C, [D,] H, W)
                s0  = seg_np[0]            # (1-or-C, [D,] H, W)  or scalar-seg

                # channel 0 of the image
                img_vol = d0[0].astype(np.float32)
                img_sitk = sitk.GetImageFromArray(img_vol)
                sitk.WriteImage(img_sitk, join(save_dir, f'aug_batch_{i:03d}_img.nii.gz'))

                seg_vol = s0[0].astype(np.float32)
                seg_sitk = sitk.GetImageFromArray(seg_vol)
                sitk.WriteImage(seg_sitk, join(save_dir, f'aug_batch_{i:03d}_seg.nii.gz'))

        self.print_to_log_file("Done saving augmented samples.")
 # region ended
